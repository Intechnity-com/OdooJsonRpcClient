...
namespace PortaCapena.OdooJsonRpcClient
{
    public interface IOdooRepository<T> where T : IOdooModel, new()
    {
        ...
        Task<OdooResult<long>> CreateAsync(OdooCommandModel model);
        ...
        Task<OdooResult<bool>> UpdateAsync(OdooCommandModel model, long id);
        Task<OdooResult<bool>> UpdateRangeAsync(OdooCommandModel model, long[] ids);
